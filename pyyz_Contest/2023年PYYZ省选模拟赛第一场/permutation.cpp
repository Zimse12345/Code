...
void cdq(unsigned L,unsigned R){
	if(L==R)return;
	unsigned mid=(L+R)/2,len=R-L+1;
	cdq(L,mid);
	poly _F,_G;
	_F.resize(len),_G.resize(len);
	for(unsigned i=0;i<len/2;i++)_F.f[i]=F.f[L+i];
	for(unsigned i=0;i<len;i++)_G.f[i]=G.f[i];
	_F=PolyMul(_F,_G);
	for(unsigned i=mid+1;i<=R;i++){
		F.f[i]+=_F.f[i-L]*inv(i)%Mod;
		if(F.f[i]>=Mod)F.f[i]-=Mod;
	}
	cdq(mid+1,R);
	return;
...
int bg[2048],bf[2048],P;

inline ll ffpow(ll x,ll y){ll res=1;while(y){if(y&1)res=res*x%P;x=x*x%P,y/=2;}return res;}
inline ll iinv(ll x){return ffpow(x,P-2);}

signed main(){
	init();
	unsigned n=read()+1,m=1;
	P=read();
	if(n<=2001){
		for(int i=1;i<=n;i++)bg[i]=ffpow(i+1,i-1);
		bf[0]=1;
		for(int i=1;i<=n;i++){
			for(int j=1;j<=i;j++){
				(bf[i]+=bf[i-j]*bg[j])%=P;
		...
		for(int i=1;i<n;i++)bf[n-1]=bf[n-1]*i%P;
		printf("%lld\n",bf[n-1]);
		return 0;
	}
	while(m<n)m*=2;
	F.resize(m),G.resize(m),F.f[0]=1;
	for(unsigned i=1;i<n;i++)G.f[i]=fpow(i+1,i-1);
	cdq(0,m-1);
	F.resize(n);
	int ans=F.f[n-1];
