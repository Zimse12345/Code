...
#define t tree[id]
#define Lt tree[tree[id].l]
#define Rt tree[tree[id].r]
...
void update(int id){t.siz=Lt.siz+Rt.siz+1;return;}
...
int kth(int id,int k){
	if(k<=Lt.siz)return kth(t.l,k);
	if(k>Lt.siz+1)return kth(t.r,k-Lt.siz-1);
	return id;
}
...
int main(){
	...
	for(int i=1,l,r,p,op,x;i<=n;i++){
		op=read(),x=read();
		if(op==1)split(rt,x,l,r),tree[++cnt]=node(0,0,x,rand(),1),rt=merge(merge(l,cnt),r);
