...
void dfs(int u,int presum){
	presum+=b[u];
	update(0,n,1,dep[u],presum);
	_max(ans,presum-query(0,n,1,dep[u]-m,dep[u]));
...
signed main(){
	...
	for(int L=n,R=n;L>=1;L--){
		while(L<R&&a[R-1]-a[L]>=k)--R;
		if(a[R]-a[L]>=k)nxt[L]=R;
		g[nxt[L]].push_back(L);
	}
	dfs(0,0);
	_write(ans);
