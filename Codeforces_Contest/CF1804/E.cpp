...
int n,a[N],p[N],q[N],pos[N],v[N],s;

void fix(int x,int y){
	int d=q[x]^v[x],t=pos[d];
	if(t==x)return;
	if(t==y)swap(p[x],p[y]),swap(pos[p[x]],pos[p[y]]);
	else{
		swap(pos[d],pos[p[x]]);
		swap(p[t],p[x]),swap(q[t],q[y]),fix(t,y);
	}
	return;
}
...
signed main(){
	n=(1<<read());
	for(int i=0;i<n;i++)a[i]=read(),p[i]=q[i]=pos[i]=i,s^=a[i];
	if(s){
		printf("Fou\n");
		return 0;
	}
	for(int i=0;i+1<n;i++)if(v[i]!=a[i]){
		int d=v[i]^a[i];
		v[i]^=d,v[i+1]^=d;
		fix(i,i+1);
	}
	printf("Shi\n");
	for(int i=0;i<n;i++)write_(p[i]);
	pc(10);
	for(int i=0;i<n;i++)write_(q[i]);
	pc(10);
    return 0;
}
