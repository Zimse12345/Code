...
int n,m,Ans,F,dd[N],tot,Tot;
map<int,int> mp;

struct node{
    int a,b,c,d,val,ans,aa,f;
...
inline void cl(int x){for(;x<N;ub(x))mx[x]=ff[x]=0;return;}

inline void upt(int x,int y,int z){
    for(;x<N;ub(x)){
        if(y>mx[x])mx[x]=y,ff[x]=0;
        if(y==mx[x])addmod(ff[x],z);
    }
    return;
}
...
inline int qmx(int x){
    int res=0;
    for(;x;db(x))_max(res,mx[x]);
    return res;
}

inline int qf(int x,int Mx){
    int res=0;
    for(;x;db(x))if(mx[x]==Mx)addmod(res,ff[x]);
    return res;
}
...
void cdq2(int L,int R){
    ...
    for(int i=M+1;i<=R;i++)if(p[i].aa){
        while(pos<=M&&p[pos].c<=p[i].c){
            if(!p[pos].aa)upt(p[pos].d,p[pos].ans,p[pos].f);
            ++pos;
        }
        
        int Mx=qmx(p[i].d);
        if(Mx+p[i].val>p[i].ans)p[i].ans=Mx+p[i].val,p[i].f=0;
        if(Mx+p[i].val==p[i].ans)addmod(p[i].f,qf(p[i].d,Mx));
    }
...
signed main(){
    ...
    for(int i=1,a,b,c,d,val;i<=n;i++){
        a=read(),b=read(),c=read(),d=read(),val=read();
        p[i]=node(a,b,c,d,val,val);
        p[i].f=1,dd[i]=d;
    }
    sort(dd+1,dd+n+1),dd[0]=-1;
    for(int i=1;i<=n;i++)if(dd[i]!=dd[i-1])mp[dd[i]]=++Tot;
    for(int i=1;i<=n;i++)p[i].d=mp[p[i].d];
    sort(p+1,p+n+1,cmp_a);
    for(int i=1;i<=n;i++)p[i].a=i;
    cdq(1,n);
    for(int i=1;i<=n;i++)_max(Ans,p[i].ans);
    for(int i=1;i<=n;i++)if(Ans==p[i].ans)addmod(F,p[i].f);
    _write(Ans);
    _write(F);
    return 0;
}
