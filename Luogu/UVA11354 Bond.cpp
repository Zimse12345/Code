...
int n,m,q,fa[N],cnt[N],ct,p[N],tag;
...
signed main(){while(scanf("%d%d",&n,&m)==2){
	if(tag++)printf("\n");
	for(int i=1;i<=ct;i++){
		for(int j=0;j<=17;j++)Fa[i][j]=fw[i][j]=0;
		vis[i]=0,hd[i]=0;
	}
	ct=n;
	for(int i=1;i<=n;i++)fa[i]=p[i]=i,cnt[i]=1;
	while(ec)nxt[ec]=to[ec]=c[ec]=0,--ec;
	...
	q=read();
	for(int i=1;i<=17;i++)for(int j=1;j<=ct;j++)Fa[j][i]=Fa[Fa[j][i-1]][i-1],fw[j][i]=max(fw[j][i-1],fw[Fa[j][i-1]][i-1]);
	while(q--){
		int x=read(),y=read(),ans=0;
		if(x==y)printf("0\n");
		else{
			if(dep[x]<dep[y])swap(x,y);
			for(int i=17;i>=0;i--)if(dep[Fa[x][i]]>=dep[y])ans=max(ans,fw[x][i]),x=Fa[x][i];
			for(int i=17;i>=0;i--)if(Fa[x][i]^Fa[y][i])ans=max(ans,max(fw[x][i],fw[y][i])),x=Fa[x][i],y=Fa[y][i];
			if(x^y)ans=max(ans,max(fw[x][0],fw[y][0]));
			printf("%d\n",ans);
		}
	}
	}return 0;
