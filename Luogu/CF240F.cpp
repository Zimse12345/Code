...
int main(){
	...
	for(char i='a';i<='z';i++){
		tree[i-'a'].cl=i;
		tree[i-'a'].build(1,n,1);
	}
	for(int i=1,l,r,f;i<=m;i++){
		l=read(),r=read(),f=0;
		for(int j=0;j<26;j++){
			tot[j]=tree[j].query(1,n,1,l,r);
			if(tot[j]&1)++f;
		}
		if(f>1)continue;
		for(int j=0;j<26;j++)tree[j].update(1,n,1,l,r,0);
		for(int j=0;j<26;j++)if(tot[j]>0){
			tree[j].update(1,n,1,l,l+tot[j]/2-1,1);
			tree[j].update(1,n,1,r-tot[j]/2+1,r,1);
			l+=tot[j]/2,r-=tot[j]/2;
		}
		for(int j=0;j<26;j++)if(tot[j]&1)tree[j].update(1,n,1,l,r,1);
	}
	for(int i=1;i<=n;i++){
		for(int j=0;j<26;j++)if(tree[j].query(1,n,1,i,i)>0){
			printf("%c",'a'+j);
			break;
		}
	}
