...
signed main(){
    ...
    for(int i=1;i<=n;i++)pre[i]=max(pre[i-1],a[i].x-b[i]);
    for(int i=n;i>=1;i--)suf[i]=max(suf[i+1],a[i+1].x-b[i]);
    for(int i=1;i<=n+1;i++)ans[a[i].y]=max(pre[i-1],suf[i]);
    for(int i=1;i<=n+1;i++)write_(ans[i]);
    pc(10);
    return 0;
}
