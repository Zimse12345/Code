...
int n,m,ve[N],dfn[N],low[N],vis[N],tot,st[N],cnt,cl[N],cn,ans;
int vl[N],f[N];
...
int main(){
	n=read(),m=read();
	for(int i=1;i<=n;i++)ve[i]=read();
	for(int i=1,u,v;i<=m;i++)u=read(),v=read(),g[u].push_back(v);
	...
	for(int i=1;i<=cn;i++){
		for(int j=0;j<int(p[i].size());j++){
			...
			for(int k=0;k<int(g[u].size());k++){
				int v=g[u][k];
				if(cl[v]!=i)G[i].push_back(cl[v]);
			}
		}
	}
	
	for(int i=1;i<=cn;i++)if(!f[i])dfs(i);
