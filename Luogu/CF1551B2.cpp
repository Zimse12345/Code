#include <cstdio>
#include <algorithm>
#include <cstring>
#include <vector>
#include <cmath>
#include <set>
#include <map>
#include <queue>
#include <stack>
#include <iostream>
#include <cstdlib>
#include <ctime>

#define ll long long
#define ull unsigned long long

using namespace std;

int read(){int qrx=0,qry=1;char qrc=' ';
while(qrc<'0'||qrc>'9'){qrc=getchar();if(qrc=='-'){qry=-1,qrc=getchar();break;}}
while(qrc>='0'&&qrc<='9')qrx=qrx*10+qrc-'0',qrc=getchar();return qrx*qry;}

const int N=2e5+7,Mod=998244353,INF=2e9+7;
int T,n,k,t[N],a[N],cl[N];
vector<int> v[N];

int main(){
	T=read();
	while(T--){
		n=read(),k=read();
		int ans=0,p=0,rm=0,y=1;
		for(int i=1;i<=n;i++){
			a[i]=read();
			v[a[i]].push_back(i);
		}
		for(int i=1;i<=n;i++)if(v[a[i]].size()<k)++rm;
		if(rm<k)y=0;
		
		for(int i=1;i<=n;i++)if(v[a[i]].size()>0){
			if(v[a[i]].size()>=k){
				for(int j=0;j<k;j++)cl[v[a[i]][j]]=j+1;
				for(int j=k;j<v[a[i]].size();j++)cl[v[a[i]][j]]=0;
			}
			else{
				for(int j=0;j<v[a[i]].size();j++){
					++p;
					if(p>k){
						p=1;
						if(rm<k)y=0;
					}
					if(y==0)cl[v[a[i]][j]]=0;
					if(y)cl[v[a[i]][j]]=p,--rm;
				}
			}
			v[a[i]].resize(0);
		}
		for(int i=1;i<=n;i++)printf("%d ",cl[i]);
		printf("\n");
	}
	return 0;
}
