...
int n,f[N][N],g[N][N],ans=INF,s;

int main(){
    n=read();
    for(int i=1,h,t;i<=n;i++)h=read(),t=read(),a[i]=book(h,t);
    sort(a+1,a+n+1);
    for(int l=0;l<=2100;l++)for(int r=0;r<=2100;r++)f[l][r]=INF;
    f[a[1].t][0]=a[1].h,s=a[1].t;
    for(int i=2;i<=n;i++){
        for(int l=0;l<=2100;l++)for(int r=0;r<=2100;r++)g[l][r]=f[l][r],f[l][r]=INF;
        for(int l=0;l<=2100;l++)for(int r=0;r<=2100;r++){
    ...
    for(int i=1;i<=2100;i++)for(int j=1;j<=2100;j++){
