...
int dfn[N],low[N],st[N],top,ct,cl[N],scc,sz[N],K,C;
vector<int> g[N];
...
void dp(int u){
	...
	for(unsigned i=0;i<g[u].size();i++){
		...
		if(i==0||v!=g[u][i-1]){
			dp(v);
			if(mx[v]+sz[u]>mx[u])mx[u]=mx[v]+sz[u],cnt[u]=cnt[v];
			else if(mx[v]+sz[u]==mx[u])(cnt[u]+=cnt[v])%=P;
		}
	}
	K=max(K,mx[u]);
	return;
...
signed main(){
	...
	for(int i=1;i<=scc;i++)if(mx[i]==K)(C+=cnt[i])%=P;
	printf("%d\n%d\n",K,C);
	return 0;
}
